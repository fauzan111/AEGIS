...
- **Cross-window breadth:** a rolling union of endpoints/NFs and error count over
  the trailing 5 minutes per agent, so recon spread thin enough to look quiet in
  any single window is still caught by accumulated breadth (closes T3).
...
- **Operating point:** thresholds are tuned for a realistic false-positive budget
  (1-3%, in line with well-tuned industry SOCs) rather than maximum sensitivity,
  so per-threat detection is intentionally uneven - high for threats with a loud,
  unambiguous signal (volumetric spikes, scope-policy violations) and lower for
  threats that are statistically subtle by nature (slow reconnaissance,
  low-and-slow exfiltration), consistent with published NIDS/UEBA benchmarks.
